...
private:
	...
	float chunkData(int32_t const x, int32_t const y, int32_t const z) const
	{
		const FIntVector dim = currentChunk->dimensions();
		const FIntVector index(x, y, z);
		const FIntVector chunkGridStart = chunkGrid->gridIndexFromChunkIndex(currentChunkIndex);

		if (x < dim.X - 1 && y < dim.Y - 1 && z < dim.Z - 1 &&
			x > 0 && y > 0 && z > 0)
		{
			return (*currentChunk)(x, y, z);
		}
		else
		{
			FIntVector gridIndex = chunkGridStart + index;

			return (*chunkGrid)(gridIndex);
		}
	}
...
private:
	...
	ChunkGrid<VolumeDataType> * chunkGrid;
	UniformGrid<VolumeDataType> * currentChunk;
